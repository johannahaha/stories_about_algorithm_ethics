...
let InformationManager = function(scene,domElement,camera,controls,informations,font,models,audios,textures,cameraHelper,cameraEye){
    ...
    let infoSegmentsHalf = [20,550,600,620,660,705,820,850,890,920,940,1000,1050,1090,1120,1200,1230,1300,1370,1410,1450,1490,1530,1580,1630,1670,1720,1790,1830];
    ...
    function manageInfo(infoNumber){
        ...
        //#endregion done
            ...
            infoOverPath(21,{useQuaternion:false,viewingDist:customViewingDist,height:10,infoRotAngle:angle,infoRotAxis:"Y",rotateCam:true,rotation:new Vector3(0,Math.PI/2,0)})
            addAxesHelper();
        }
            ...
            let rotation = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(1,0,0),Math.PI/2*3);
        ...
        //flying lower again
            ...
            tl.to(scope.camera.rotation,{
                duration: 3,
                ease: "none",
                x: THREE.MathUtils.degToRad(270) + 0.5,
                z: 0,
                onStart: scope.controls.enabled = false
            })
            ...
            tl.to(scope.camera.rotation,{
                duration:5,
                x: THREE.MathUtils.degToRad(270)
            }),"+=2";
